...
use crate::util::shared::{FORMAT_V0_BLAKE3_CONTEXT_ENCRYPTION, FORMAT_V0_BLAKE3_CONTEXT_HMAC, BUFFER_SIZE, GLOBAL_PROGRESS_STYLE};
...
use crate::commands::decrypt::decrypt::DecryptArgs;

type Aes256Ctr128LE = ctr::Ctr128LE<aes::Aes256>;



pub fn decrypt_v0(args: &DecryptArgs) -> anyhow::Result<()> {
    // Set up buffers
    ...
    let encryption_key: SecretBox<[u8; 32]> =
        SecretBox::init_with(|| blake3::derive_key(FORMAT_V0_BLAKE3_CONTEXT_ENCRYPTION, key.expose_secret()));
    let hmac_key: SecretBox<[u8; 32]> =
        SecretBox::init_with(|| blake3::derive_key(FORMAT_V0_BLAKE3_CONTEXT_HMAC, key.expose_secret()));
